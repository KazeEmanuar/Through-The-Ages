...
u32 flyAllowActionCount = sizeof(flyAllowAction) / 4;

u8 isAllowedAction() {
    u16 u;
    for (u = 0; u < flyAllowActionCount; u++) {
        if (flyAllowAction[u] == gMarioState->action) {
            return 1;
        }
    }
    return 0;
...
Gfx shoe1 = gsSPDisplayList(shoe2Wing_a3_mesh);     // link to shoe1
Gfx shoe2 = gsSPDisplayList(shoe1Wing_a3_001_mesh); // link to shoe2

Gfx backupshoe11; // backup to hat
Gfx backupshoe12;
Gfx backupshoe21; // backup to hat
Gfx backupshoe22;
u8 initShoes = 0;
...
Gfx backupHat1 = gsSPDisplayList(mario_face_part_cap_on_dl);
Gfx backupHat2 = gsSPLight(&mario_red_lights_group.l, 1);
extern const Gfx mario_m_logo_dl[];
extern const Vtx mario_m_logo_dl_vertex[];
Gfx backupHat3 = gsSPVertex(mario_m_logo_dl_vertex, 7, 0);

s32 spawnID = -1;
u16 waterTimer = 0;
u16 marioKI = 0;
u16 timerBeforeSwim = 0;
f32 spawnParticle = 0;
extern u16 shoeTimer;
void try_print_debug_mario_level_info(void) {
    Gfx *k = (Gfx *) segmented_to_virtual(&mario_face_cap_on_dl[0]);
    Gfx *l = (Gfx *) segmented_to_virtual(&mario_m_logo_dl[0]);
    struct Object *object;
    Gfx *shoe1replace = (Gfx *) segmented_to_virtual(&mario_left_foot[0]);
    Gfx *shoe2replace = (Gfx *) segmented_to_virtual(&mario_right_foot[0]);
    if (gCurrLevelNum == LEVEL_SL) {
        if (isAllowedAction()) {
            if ((gMarioState->controller->buttonPressed & A_BUTTON)
                ...
                gMarioState->faceAngle[0] = 0x0400;
                gMarioState->faceAngle[0] = 0x0400;
                gMarioState->angleVel[0] = 0;
                gMarioState->angleVel[1] = 0;
                gMarioState->angleVel[2] = 0;
            }
        }
    }
    if (!initShoes) {
        initShoes = 1;
        backupshoe11 = shoe1replace[0];
        backupshoe12 = shoe1replace[1];
        backupshoe21 = shoe2replace[0];
        backupshoe22 = shoe2replace[1];
    }
    ...
    if (shoeTimer) {
        ...
        *shoe1replace = backupshoe11;
        *(shoe1replace + 1) = backupshoe12;
        *shoe2replace = backupshoe21;
        *(shoe2replace + 1) = backupshoe22;
    }

    if (spawnID >= 0) {
        if (!cur_obj_nearest_object_with_behavior(bhvPokemon)) {
            if (gMarioState->controller->buttonPressed & L_TRIG) {
                object = spawn_object(gMarioObject, 0xf0 + spawnID, bhvPokemon);
                object->oBehParams2ndByte = spawnID;
                object->oPosY += 50.f;
                spawn_mist_particles();
                object->oForwardVel = 40.f;
                object->oFaceAnglePitch = 0;
                object->oFaceAngleRoll = 0;
            }
        }
        // print_text_fmt_int(10, 20, "POKEMON %d", spawnID);
    ...
    if (gCurrLevelNum == LEVEL_SSL) {
        if (gMarioState->action & ACT_FLAG_METAL_WATER) {
            if (gMarioState->pos[1] == gMarioState->floorHeight) {
                if ((gMarioState->forwardVel > 1.f)) {
                    if ((gMarioState->forwardVel < 48.f)) {
                        gMarioState->forwardVel += 1.5f;
                    }
                    spawnParticle += 1.f;
                    spawnParticle += gMarioState->forwardVel / 30.f;
                    if (spawnParticle > 3.f) {
                        spawnParticle -= 3.f;
                        object = spawn_object(gMarioObject, 0, bhvBreathParticleSpawner);
                        object->oMoveAngleYaw += 0x8000;
                    }
                }
            }
        }
        if (gMarioState->action & ACT_FLAG_SWIMMING) {
            ...
            if (gMarioState->pos[1] == gMarioState->floorHeight) {
                waterTimer = approach_s16_symmetric(waterTimer, 120, 2);
            }
        }
        if ((gMarioState->action == ACT_METAL_WATER_JUMP)
            || (gMarioState->action == ACT_METAL_WATER_FALLING)) {
            if (gMarioState->pos[1] > (gMarioState->floorHeight + 50.f)) {
                if (waterTimer && (gMarioState->controller->buttonPressed & A_BUTTON)) {
                    gMarioState->flags &= ~MARIO_UNDERWATER_SUIT;
    ...
    if (gCurrLevelNum != LEVEL_SL) {
        ...
        if (gMarioState->action != ACT_FLYING) {
            marioKI = approach_s16_symmetric(marioKI, 150, 2);
        }
    }
}
