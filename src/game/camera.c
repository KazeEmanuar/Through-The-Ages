...
struct PlayerCameraState *sMarioCamState = &gPlayerCameraState;

s32 update_c_up(Vec3f, Vec3f);
s32 update_in_cannon(Vec3f, Vec3f);
...
f32 calc_y_to_curr_floor(f32 *posOff, f32 posMul, f32 posBound, f32 *focOff, f32 focMul, f32 focBound) {
    ...
    if (gMarioState->pos[1] == floorHeight) {
        if (gMarioState->floor) {
            if (marioLastObj = gMarioState->floor->object) {
                lastMarioFloorHeight = floorHeight;
            } else {
                lastMarioFloorHeight = -999999.f;
            }
        }
    ...
    if (floorHeight < lastMarioFloorHeight) {
        floorHeight = lastMarioFloorHeight;
    }

    if (!(gMarioState->action & ACT_FLAG_METAL_WATER)) {
        if (floorHeight < gMarioState->waterLevel) {
            floorHeight = gMarioState->waterLevel;
        }
    }
...
s32 update_c_up(Vec3f focus, Vec3f pos) {
    ...
    s32 yaw = sMarioCamState->faceAngle[1] + sModeOffsetYaw + DEGREES(180);
 ...
 */
s32 update_in_cannon(Vec3f focus, Vec3f pos) {
...
void reset_camera(struct Camera *c) {
    ...
    if ((((gCurrCourseNum < 16) && (gCurrCourseNum > 0)) || gCurrLevelNum == LEVEL_CASTLE_GROUNDS)
        && (!(save_file_get_star_flags(gCurrSaveFileNum - 1, gCurrCourseNum) & 0x80))) {
...
#define approach_pos min
#define approach_posF min
/**
...
s32 get_cutscene_from_mario_status(struct Camera *c) {
    ...
    if (cutscene == 0) {
        ...
        if (sMarioCamState->cameraEvent == 0xc8) {
            cutscene = 0xc9;
        }
 ...
 */
s32 next_lakitu_state(Vec3f newPos, Vec3f newFoc, Vec3f curPos, Vec3f curFoc, Vec3f oldPos,
                      Vec3f oldFoc, s32 yaw) {
    s32 yawVelocity;
    s32 pitchVelocity;
    f32 distVelocity;
    f32 goalDist;
...
void stop_transitional_movement(void) {
    sStatusFlags &= ~(CAM_FLAG_START_TRANSITION | CAM_FLAG_TRANSITION_OUT_OF_C_UP);
    sModeTransition.framesLeft = 0;
}
...
struct CameraTrigger sCamPSS[] = { NULL_TRIGGER };
...
extern void adjustTimers(int time);
...
s32 cutscene_object_without_dialog(s32 cutscene, struct Object *o) {
    return cutscene_object_with_dialog(cutscene, o, -1);
}
...
void water_death_move_to_mario_side(struct Camera *c) {
    ...
    vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
    yaw = approach_s16_asymptotic(yaw, (sMarioCamState->faceAngle[1] - 0x3000), 8);
    vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_approach_mario(struct Camera *c) {
    ...
    c->focus[0] = approach_f32_asymptotic(c->focus[0], sMarioCamState->pos[0], 0.1f);
    c->focus[1] = approach_f32_asymptotic(c->focus[1], sMarioCamState->pos[1] + 110.f, 0.1f);
    c->focus[2] = approach_f32_asymptotic(c->focus[2], sMarioCamState->pos[2], 0.1f);
}
...
BAD_RETURN(s32) cutscene_unlock_key_door_focus_lock(UNUSED struct Camera *c) {
    sCutsceneVars[3].point[1] =
        approach_f32_asymptotic(sCutsceneVars[3].point[1], sMarioCamState->pos[1] + 140.f, 0.07f);
}
...
void zoom_fov_30(UNUSED struct MarioState *m) {
    ...
    // Nintendo.
    sFOVState.fov = approach_f32_symmetric(sFOVState.fov, 30.f, absf((30.f - sFOVState.fov) / 60.f));
}
