...
static s32 swimming_near_surface(struct MarioState *m) {
    if (m->flags & (MARIO_METAL_CAP | MARIO_UNDERWATER_SUIT)) {
        return FALSE;
    }
...
extern void clip_mario(Vec3f prev_pos, Vec3f new_pos);
...
u32 isFloorSlippery() {
    if (gMarioState->floor) {
        ...
        if (gMarioState->floor->normal.y < 0.7f) {
            return 1;
        }
    }
    return 0;
...
static s32 act_metal_water_standing(struct MarioState *m) {
    ...
    if (isFloorSlippery()) {
        m->pos[0] += m->floor->normal.x * 50.f;
        m->pos[2] += m->floor->normal.z * 50.f;
        waterTimer = 0;
        return set_mario_action(m, ACT_METAL_WATER_FALLING, 1);
    }
    return FALSE;
...
static s32 act_metal_water_walking(struct MarioState *m) {
    ...
    if ((val04 = (s32)(m->forwardVel / 8.0f * 0x10000)) < 0x1000) {
        val04 = 0x1000;
    }
...
static s32 act_metal_water_falling(struct MarioState *m) {
    ...
    // stationary_slow_down(m);
    m->vel[1] = approach_f32(m->vel[1], get_buoyancy(m), 2.0f, 1.0f);
