...
void update_flying_pitch(struct MarioState *m) {
    ...
    if (marioKI) {
        s16 targetPitchVel = -(s16) (m->controller->stickY);
    }
    if (targetPitchVel > 0) {
        if (m->angleVel[0] < 0) {
            m->angleVel[0] += 0x40;
...
void update_flying_yawKI(struct MarioState *m) {
    s16 targetYawVel = -(s16) (m->controller->stickX * (m->forwardVel / 4.0f));
    ...
    if (targetYawVel > 0) {
        if (m->angleVel[1] < 0) {
            ...
            m->angleVel[1] = approach_s32(m->angleVel[1], targetYawVel, 0x20, 0x40);
        }
    } else if (targetYawVel < 0) {
        if (m->angleVel[1] > 0) {
            m->angleVel[1] -= 0x80;
            ...
            m->angleVel[1] = approach_s32(m->angleVel[1], targetYawVel, 0x40, 0x20);
        }
    } else {
        m->angleVel[1] = approach_s32(m->angleVel[1], 0, 0x40, 0x40);
    }
...
void update_flying_pitchKI(struct MarioState *m) {
    s16 targetPitchVel = -(s16) (m->controller->stickY * (m->forwardVel / 10.0f));
    if (targetPitchVel > 0) {
        if (m->angleVel[0] < 0) {
            m->angleVel[0] += 0x80;
            if (m->angleVel[0] > 0x20) {
            ...
            m->angleVel[0] = approach_s32(m->angleVel[0], targetPitchVel, 0x40, 0x80);
        }
    } else if (targetPitchVel < 0) {
        if (m->angleVel[0] > 0) {
            m->angleVel[0] -= 0x80;
            ...
            m->angleVel[0] = approach_s32(m->angleVel[0], targetPitchVel, 0x80, 0x40);
        }
    } else {
        m->angleVel[0] = approach_s32(m->angleVel[0], 0, 0x40, 0x40);
    }
...
void update_flying(struct MarioState *m) {
    ...
    if (marioKI) {
        update_flying_pitchKI(m);
        update_flying_yawKI(m);
    } else {
...
s32 act_ride_jetski(struct MarioState *m) {
...
s32 act_flying(struct MarioState *m) {
    ...
    if (!(m->flags & MARIO_WING_CAP) && (!onYoshi) && (!marioKI)) {
        if (m->area->camera->mode == CAMERA_MODE_BEHIND_MARIO) {
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_RIDE_JETSKI:              cancel = act_ride_jetski(m);      break;
