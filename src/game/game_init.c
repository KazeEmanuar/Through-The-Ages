...
void my_rdp_init(void) {
    osViModeTable[OS_VI_NTSC_LAN1].comRegs.vSync = 2100;
...
void end_master_display_list(void) {
    ...
    if (gCurrLevelNum == LEVEL_SSL) {
        gDPPipeSync(gDisplayListHead++);
        gDPSetFillColor(gDisplayListHead++, GPACK_RGBA5551(64,64,255,1));
        gDPFillRectangle(gDisplayListHead++, 10, YPOS, waterTimer+10, YPOS + 2);
        if (waterTimer != 120) {
            gDPPipeSync(gDisplayListHead++);
            gDPSetFillColor(gDisplayListHead++, 0x0000);
            gDPFillRectangle(gDisplayListHead++, waterTimer+10, YPOS, 130, YPOS + 2);
        }
    }
    if (gCurrLevelNum == LEVEL_SL) {
        gDPPipeSync(gDisplayListHead++);
        gDPSetFillColor(gDisplayListHead++, GPACK_RGBA5551(255,255,0,1));
        gDPFillRectangle(gDisplayListHead++, 10, YPOS, marioKI + 10, YPOS + 2);
        if (marioKI != 150) {
            gDPPipeSync(gDisplayListHead++);
            gDPSetFillColor(gDisplayListHead++, 0x0000);
            gDPFillRectangle(gDisplayListHead++, marioKI + 10, YPOS, 160, YPOS + 2);
        }
    }
