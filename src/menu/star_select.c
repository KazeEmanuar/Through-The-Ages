...
void render_100_coin_star(u8 stars) {
    if (stars & (1 << 6)) {
        ...
        sStarSelectorModels[6] = spawn_object_abs_with_rot(gCurrentObject, 0, 0,
...
void bhv_act_selector_init(void) {
    ...
    //giving these a model ID crashes? kazetodo
    ...
    render_100_coin_star(stars); //crashes only if 100c star collected
}
...
s32 lvl_update_obj_and_load_act_button_actions(UNUSED s32 arg, UNUSED s32 unused) {
    if (sActSelectorMenuTimer >= 11) {
        ...
        if ((gPlayer3Controller->buttonPressed & (A_BUTTON | START_BUTTON | B_BUTTON | Z_TRIG))) {
            ...
            if (sSelectedActIndex>0){
                if (gCurrLevelNum == LEVEL_WDW){
    sWarpDest.areaIdx = 2;
    sWarpDest.nodeId = 0x0C;
                }
            }
        }
    }
