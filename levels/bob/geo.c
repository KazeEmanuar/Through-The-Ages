#include <ultra64.h>
#include "sm64.h"
#include "geo_commands.h"

#include "game/level_geo.h"
#include "game/geo_misc.h"
#include "game/camera.h"
#include "game/moving_texture.h"
#include "game/screen_transition.h"
#include "game/paintings.h"

#include "make_const_nonconst.h"

#include "levels/bob/header.h"

#include "levels/bob/chain_chomp_gate/geo.inc.c"
#include "levels/bob/seesaw_platform/geo.inc.c"
#include "levels/bob/grate_door/geo.inc.c"
#include "levels/bob/areas/1/geo.inc.c"
#include "levels/bob/geo.inc.c"

#include "levels/bob/noseguy/geo.inc.c"
#include "levels/bob/rockforchase/geo.inc.c"
#include "levels/bob/noseguyrock/geo.inc.c"
#include "levels/bob/fliptiles/geo.inc.c"
#include "levels/bob/quetzl/geo.inc.c"