...
static void newcam_rotate_button(void) {
    ...
    if (((~CBUTTONALLOWED) & 0x0f) & (gPlayer1Controller->buttonPressed)) {
...
static void newcam_collision(void) {
    ...
    camdir[0] = newcam_pos[0] - newcam_pos_target[0];
    camdir[1] = newcam_pos[1] - newcam_pos_target[1];
    camdir[2] = newcam_pos[2] - newcam_pos_target[2];
...
#define CAMERA_TERRAIN_GOTHROUGH 7777
#define CAMERA_TERRAIN_NOSURFBACK 7777
    find_surface_on_ray(noColRadius, camdirback, &surfback, &hitposback);
    if (surfback) {
    ...
    find_surface_on_ray(calcPos, camdir, &surf, &hitpos);
        ...
        if (gMarioState->action == ACT_WALL_KICK_AIR) {
            if (gMarioState->actionTimer < WKROTTIME) {
                if (wkHit) {
                    ...
                    find_surface_on_ray(wkStart, dir, &WKSURF, hit_pos);
                    if (WKSURF) {
...
extern s32 update_c_up(Vec3f, Vec3f);
s32 mode_c_up_camera(struct Camera *c) {
    ...
    if (!(gCameraMovementFlags & CAM_MOVE_STARTED_EXITING_C_UP)) {
        // Normal update
        stop_transitional_movement(); // CuckyDev: Fix C-Up randomly taking a long time to transition
        ...
        if (sStatusFlags & CAM_FLAG_TRANSITION_OUT_OF_C_UP) {
            ...
            sMarioCamState->headRotation[0] =
                approach_s16_symmetric(sMarioCamState->headRotation[0], 0, 1024);
            sMarioCamState->headRotation[1] =
                approach_s16_symmetric(sMarioCamState->headRotation[1], 0, 1024);
        } else {
